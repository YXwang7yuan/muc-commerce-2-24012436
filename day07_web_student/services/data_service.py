...
def load_dashboard_data(base_dir: Path, selected_category: str = "全部") -> dict:
    ...
    # DONE 2-1：在已有两张指标卡基础上，增加“总体流失率”和“平均订单数”。
    metrics = [
    ...
    # DONE 2-2：找出流失率最高的生命周期阶段，并生成一句数据观察。
    max_loss_row = segment_df.loc[segment_df["流失率"].idxmax()]
    max_stage = max_loss_row.get("TenureGroup", "未知")
    max_loss_rate = max_loss_row["流失率"]
    max_boundary = max_loss_row["平均距上次下单天数"] 
    insight = f"生命周期阶段中{max_stage}流失风险最高，流失率达到{max_loss_rate:.1%}，平均距上次下单天数为{max_boundary:.1f}天，需重点运营干预。"
