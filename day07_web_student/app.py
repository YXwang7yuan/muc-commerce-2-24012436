...
def download():
    ...
    category_df["流失率"] = category_df["流失率"].map(lambda value: f"{value:.1%}")
    category_df["平均订单数"] = category_df["平均订单数"].map(lambda value: f"{value:.2f}")

    output = StringIO()
    category_df.to_csv(output, index=False, encoding="utf-8-sig")
    output.seek(0)

    filename = f"category_{category}.csv" if category != "全部" else "category_all.csv"
